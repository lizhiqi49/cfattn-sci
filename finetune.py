...
from models.attention_processor import (
    ...
    XFormersCrossFrameAttnProcessor,
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    # If xformers available, use it
    if is_xformers_available():
        import xformers
        unet.enable_xformers_memory_efficient_attention()
        cross_frame_attn_procs_cls = XFormersCrossFrameAttnProcessor
    else:
        cross_frame_attn_procs_cls = CrossFrameAttnProcessor

    # Replace cross frame attention processor
    cross_frame_attn_processor = cross_frame_attn_procs_cls(video_length=video_length)
    params = set_cross_frame_attn_processor(
        unet, cross_frame_attn_processor,
        requires_grad=True, torch_dtype=torch.float32, return_params=True
    )

    # Dataset
    train_dset = RGBVideoDataset(
        root="./dataset",
        split="train",
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dloader):
            ...
            with accelerator.accumulate(unet):
                ...
                x_ce = rearrange(x_ce, 'b n c h w -> (b n) c h w')
